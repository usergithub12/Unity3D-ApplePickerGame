...
public class AppleTree : MonoBehaviour
{
    ...
    void Update()
    {
        if (changeToChangeDirections > 1f)
            changeToChangeDirections+=0.12f;
        // changeToChangeDirections += Time.deltaTime;
        if (secondsBetweenAppleDrops > 0.1f)
            secondsBetweenAppleDrops-=0.0001f;
